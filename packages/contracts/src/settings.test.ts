...
describe("ClientSettings terminal view mode", () => {
  it("defaults legacy settings documents to tabs view mode", () => {
    const decoded = decodeClientSettings({});

    expect(decoded.terminalViewMode).toBe("tabs");
    expect(DEFAULT_CLIENT_SETTINGS.terminalViewMode).toBe("tabs");
  });

  it("accepts terminal view mode patches", () => {
    expect(decodeClientSettingsPatch({}).terminalViewMode).toBeUndefined();
    expect(decodeClientSettingsPatch({ terminalViewMode: "sidebar" })).toEqual({
